    ...
    >
      <div className="text-right">
        <span
          ...
          style={{ color: higher === "jp" ? jpColor : "rgba(255,255,255,0.8)" }}
        >
          {jpVal}
        </span>
      </div>
      ...
      <div className="text-left">
        <span
          className="text-[13px] font-bold"
          style={{ color: higher === "ar" ? arColor : "rgba(255,255,255,0.8)" }}
        >
          {arVal}
        </span>
      </div>
  ...
  return (
    <div className="space-y-12 pb-16">
      ...
      <motion.div
        ...
        style={{
          background: "linear-gradient(135deg, rgba(29,185,84,0.08) 0%, rgba(0,0,0,0) 50%, rgba(232,17,156,0.08) 100%)",
        }}
      >
        ...
        <div className="relative flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <div className="text-center md:text-right flex-1">
            <div className="text-5xl mb-2">{JP.emoji}</div>
          ...
          <div className="text-center md:text-left flex-1">
            <div className="text-5xl mb-2">{AR.emoji}</div>
      ...
      {/* Radar */}
      <SectionLabel icon="🕸" color="#ffffff">Perfil de Oyente</SectionLabel>
      <Card>
        <CardHeader>
      ...
      {/* Dual timeline */}
      <SectionLabel icon="📈" color="#ffffff">Evolución Paralela</SectionLabel>
      <Card>
        <CardHeader>
        ...
        <CardContent>
          <DualTimelineChart
            jpData={Object.entries(JP.monthly).map(([k, v]) => ({ month: k, hours: v }))}
            arData={Object.entries(AR.monthly).map(([k, v]) => ({ month: k, hours: v }))}
          />
        </CardContent>
      </Card>

      {/* Weekday + Hourly */}
      <SectionLabel icon="📅" color="#ffffff">Patrones Comparados</SectionLabel>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          <CardHeader><CardTitle>Horas por Día de la Semana</CardTitle></CardHeader>
          <CardContent>
            <WeekdayChart
              jpData={JP.weekdayHours}
              arData={AR.weekdayHours}
            />
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <DualHourlyChart
              jpData={JP.hourlyHours}
              arData={AR.hourlyHours}
            />
          </CardContent>
        </Card>
      </div>
      ...
      {/* Stats table */}
      <SectionLabel icon="📋" color="#ffffff">Tabla Comparativa</SectionLabel>
      <Card>
        <CardHeader>
        ...
        <CardContent>
          ...
          <VSRow label="Mes pico" jpVal={`${JP.peaks.peakMonth}`} arVal={`${AR.peaks.peakMonth}`} jpColor={jpColor} arColor={arColor} higher="neutral" />
        </CardContent>
      </Card>

      {/* Key differences */}
      <SectionLabel icon="⚡" color="#ffffff">Diferencias Clave</SectionLabel>
