    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div
          ...
          style={{ background: activeTab === "jp" ? JP.color : activeTab === "ar" ? AR.color : "linear-gradient(135deg, #1DB954, #E8119C)" }}
        />
        <div
          className="absolute -bottom-32 -right-32 w-96 h-96 rounded-full blur-3xl opacity-[0.04] transition-all duration-1000"
          style={{ background: activeTab === "ar" ? AR.color : activeTab === "jp" ? JP.color : "#E8119C" }}
        />
      </div>
      ...
      <header className="sticky top-0 z-40 border-b border-white/[0.06] bg-[#0a0a0a]/80 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="text-[11px] text-white/20 hidden sm:block">
              Datos 2015 – 2026
            </div>
          </div>
        </div>
      ...
      <main className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Tabs.Root value={activeTab} onValueChange={setActiveTab}>
          {/* Tab list */}
          <Tabs.List className="flex gap-2 mb-10 p-1 rounded-2xl bg-white/[0.03] border border-white/[0.06] w-fit mx-auto">
            {TABS.map((tab) => (
              <Tabs.Trigger
                key={tab.id}
                value={tab.id}
                className="relative flex items-center gap-2 px-5 py-2.5 rounded-xl text-[13px] font-medium transition-all duration-200 outline-none cursor-pointer text-white/40 hover:text-white/70 data-[state=active]:text-white"
              >
                {activeTab === tab.id && (
                  <motion.div
          ...
          {/* Tab content */}
          <Tabs.Content value="jp" forceMount className={activeTab !== "jp" ? "hidden" : ""}>
            <AnimatePresence mode="wait">
              {activeTab === "jp" && (
                ...
                >
                  <PersonSection person={JP} />
                </motion.div>
              )}
            </AnimatePresence>
          </Tabs.Content>

          <Tabs.Content value="ar" forceMount className={activeTab !== "ar" ? "hidden" : ""}>
            <AnimatePresence mode="wait">
              {activeTab === "ar" && (
                <motion.div
                  key="ar"
                  ...
                  transition={{ duration: 0.4 }}
                >
                  <PersonSection person={AR} />
                </motion.div>
              )}
            </AnimatePresence>
          </Tabs.Content>

          <Tabs.Content value="compare" forceMount className={activeTab !== "compare" ? "hidden" : ""}>
            <AnimatePresence mode="wait">
              {activeTab === "compare" && (
                <motion.div
                  key="compare"
